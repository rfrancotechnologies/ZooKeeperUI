...
public class ZooKeeperServiceTest {
    ...
    public void SuccessfulNodeExport() throws Exception {
        ...
        NodeExport expectedExport = new NodeExport().name("").value("");
    ...
    @Test
    public void SuccessfulRetrievalOfNodeDataType() throws Exception {
        String testPath = "/test/path";

        doReturn("Json".getBytes()).when(getDataBuilder).forPath("/.node_data_types" + testPath);
        NodeDataType obtainedDataType = zooKeeperService.getNodeDataType(testPath);
        Assert.assertEquals(NodeDataType.JSON, obtainedDataType);
    }

    @Test
    public void DefaultRetrievedNodeDataTypeIsString() throws Exception {
        String testPath = "/test/path";

        doThrow(new NotFoundException("Test message", null)).when(getDataBuilder).forPath("/.node_data_types" + testPath);
        NodeDataType obtainedDataType = zooKeeperService.getNodeDataType(testPath);
        Assert.assertEquals(NodeDataType.STRING, obtainedDataType);
    }

    @Test
    public void SuccessfulSettingOfNodeDataType() throws Exception {
        String testPath = "/test/path";

        zooKeeperService.setNodeDataType(testPath, NodeDataType.JSON);
        verify(setDataBuilder, times(1)).forPath("/.node_data_types" + testPath, "Json".getBytes("UTF-8"));
    }
